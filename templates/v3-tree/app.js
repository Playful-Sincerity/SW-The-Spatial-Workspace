...
// ── Layout Engine (v3-tree — Packed Balloon Tree) ──────────────────────
// Deterministic, physics-free placement. Each subtree is a packed circle
// ("balloon"); children pack around parents at variable radii; sibling
// subtree circles cannot overlap by construction (angular slots allocated
// by subtree footprint, not by count); trunk crossings prevented by the
// half-plane constraint (non-root children fan only into the outward arc
// centered on the inherited direction from parent).
//
// See SPEC-TREE.md for the full mechanism, constants, and rationale.

// Vestigial physics knobs (kept so the old Physics Slider Panel init code
// doesn't throw on boot; sliders are inert in v3-tree).
...
// Packed-balloon constants. Tune here, not in physics panel.
const BALLOON = {
  padding:     30,    // gap between sibling subtree circles, px
  rootArcDeg:  360,   // root's children fan around full circle
  outwardArcDeg: 240, // non-root children fan into this outward arc (centered on inherited direction)
  minRingScale: 1.15, // ringRadius multiplier when cluster is small (≤3 children)
  maxPush:     30,    // cap on cousin-overlap nudge, px
  weightK:     0,     // per-child radial push from sqrt(leafCount) — disabled, was amplifying spread
};

// Root orientation (cosmetic — canvas pans/zooms anyway). -90° = "up".
const ROOT_DIRECTION_DEG = -90;
...
function createLayout({ baseSpacing = 180 } = {}) {
  ...
  // rect's diagonal so circles inscribe the box (slightly conservative,
  // avoids label-corner clipping).
  ...
  // Bottom-up: compute subtreeRadius for every node.
  // subtreeRadius = the radius of the minimal circle enclosing this node
  // AND its entire (visible) subtree. Leaves: their own button circle.
  // Interior nodes: max over children of (childDistance + child.subtreeRadius).
  //
  // Variable-distance packing: each child sits at its own distance from
  // parent, not on a shared ring. Small-subtree children sit tight against
  // the parent; heavy-subtree children push out only as far as they must.
  // This is what produces dense leaf packing instead of a uniform fan.
  //
  // We compute everything in this pass as if this node is a non-root
  // (conservative: outwardArcDeg). Root will re-compute with 360° during
  // placement. That means root's subtreeRadius is a slight overestimate —
  // harmless.
  function computeSubtreeRadius(d) {
    const nr = nodeVisualRadius(d);
    const children = d.children || [];
    if (children.length === 0) {
      d._subtreeRadius = nr + BALLOON.padding / 2;
      return d._subtreeRadius;
    }
    for (const c of children) computeSubtreeRadius(c);

    const packing = packChildren(d, children, BALLOON.outwardArcDeg);
    let maxReach = 0;
    for (const p of packing) {
      const reach = p.dist + p.child._subtreeRadius;
      if (reach > maxReach) maxReach = reach;
    }
    d._subtreeRadius = maxReach + BALLOON.padding / 2;
    return d._subtreeRadius;
  }

  // Core packing math: given a parent node and its children and the
  // available arc (degrees), return each child's (dist, slotRad) so that
  // all children fit within the arc without their subtree bounding
  // circles overlapping each other.
  //
  // Step 1: each child sits at its minimum-safe distance from parent:
  //         dist = parent.nodeRadius + child.subtreeRadius + padding
  //         Its angular slot at that distance: 2*asin(subtreeR / dist)
  // Step 2: if sum of slots > available arc, binary-search a uniform
  //         scale factor on distances until slots fit.
  //
  // Result: children with small subtrees sit close to parent (short
  // branches, dense leaves). Children with heavy subtrees sit far out
  // (long branches). Trees pack organically.
  function packChildren(parent, children, availableDeg) {
    const nr = nodeVisualRadius(parent);
    const minDist = BALLOON.padding;
    const availableRad = availableDeg * Math.PI / 180;

    // Weight contribution from descendant count — pushes collapsed "heavy"
    // subtrees (e.g., memory/39 files) further from parent than "light"
    // leaves. Creates visible radial variance even when all children are
    // collapsed to same-sized buttons. Uses raw subtree leafCount via
    // d.data._children (collapsed) OR _leafCount (expanded).
    function weightRadius(c) {
      let count = 1;
      if (c.data && Array.isArray(c.data._children)) count = c.data._children.length || 1;
      if (c._leafCount && c._leafCount > count) count = c._leafCount;
      return BALLOON.weightK * Math.sqrt(count);
    }

    const items = children.map(c => {
      const R = c._subtreeRadius;
      const w = weightRadius(c);
      const baseDist = Math.max(nr + R + minDist + w, nr * BALLOON.minRingScale + R);
      return { child: c, R, baseDist };
    });

    // Compute slot widths at the base distances.
    function totalSlotAt(scale) {
      let sum = 0;
      for (const it of items) {
        const d = it.baseDist * scale;
        const ratio = Math.min(0.9999, it.R / d);
        sum += 2 * Math.asin(ratio);
      }
      return sum;
    }

    // If the base packing fits, use it.
    let scale = 1;
    if (totalSlotAt(1) > availableRad) {
      // Binary-search for a scale that makes total slot == availableRad.
      let lo = 1, hi = 50;
      for (let i = 0; i < 32; i++) {
        const mid = (lo + hi) / 2;
        if (totalSlotAt(mid) > availableRad) lo = mid; else hi = mid;
      }
      scale = hi;
    }

    for (const it of items) {
      it.dist = it.baseDist * scale;
      it.slotRad = 2 * Math.asin(Math.min(0.9999, it.R / it.dist));
    }
    return items;
  }

  // Top-down: place node's children around it using packChildren.
  //   - Root: 360° arc centered on ROOT_DIRECTION_DEG.
  //   - Non-root: outwardArcDeg centered on the node's inherited _direction
  //     (prevents trunk crossings — children never point back through parent).
  //   - Ordering: largest subtrees at center of arc, smaller flanking.
  function placeChildrenOf(d, isRoot) {
    const children = d.children || [];
    if (children.length === 0) return;

    const availableDeg = isRoot ? BALLOON.rootArcDeg : BALLOON.outwardArcDeg;
    const items = packChildren(d, children, availableDeg);

    // Sort DESC by subtreeRadius and fold into a center-outward ordering.
    items.sort((a, b) => b.R - a.R);
    const ordered = [];
    items.forEach((it, i) => {
      if (i === 0) ordered.splice(Math.floor(ordered.length / 2), 0, it);
      else if (i % 2 === 1) ordered.push(it);
      else ordered.unshift(it);
    });

    const totalSlotRad = ordered.reduce((s, it) => s + it.slotRad, 0);
    const centerRad = (isRoot ? ROOT_DIRECTION_DEG : d._direction) * Math.PI / 180;
    let cumRad = centerRad - totalSlotRad / 2;

    for (const it of ordered) {
      const childAngleRad = cumRad + it.slotRad / 2;
      it.child.x = d.x + it.dist * Math.cos(childAngleRad);
      it.child.y = d.y + it.dist * Math.sin(childAngleRad);
      it.child._direction = childAngleRad * 180 / Math.PI;
      cumRad += it.slotRad;
    }

    for (const child of children) placeChildrenOf(child, false);
  }

  // Cousin-overlap sanity pass. Rare with subtreeRadius allocation, but
  // small numerical drift in deep-skewed trees can nudge cousins close.
  // Bounded single-pass nudge: for each node, if its visual circle overlaps
  // a non-ancestor's visual circle, push both along their directions by
  // maxPush/2 each. Stored positions sacred — only applied once at placement.
  function resolveCousinOverlaps() {
    const N = _nodes.length;
    if (N < 2) return;
    const pushCap = BALLOON.maxPush;

    // Build ancestor sets for fast "are these related" check
    const ancestors = new Map();
    for (const d of _nodes) {
      const set = new Set();
      let p = d.parent;
      while (p) { set.add(p); p = p.parent; }
      ancestors.set(d, set);
    }

    for (let i = 0; i < N; i++) {
      const a = _nodes[i];
      const ra = nodeVisualRadius(a);
      for (let j = i + 1; j < N; j++) {
        const b = _nodes[j];
        // Skip ancestor/descendant pairs
        if (ancestors.get(a).has(b) || ancestors.get(b).has(a)) continue;
        // Skip siblings (their placement is already non-overlapping by wedge)
        if (a.parent === b.parent) continue;
        const rb = nodeVisualRadius(b);
        const dx = b.x - a.x, dy = b.y - a.y;
        const dist = Math.hypot(dx, dy);
        const overlap = (ra + rb + BALLOON.padding / 2) - dist;
        if (overlap > 0 && dist > 0.001) {
          const nx = dx / dist, ny = dy / dist;
          const push = Math.min(pushCap, overlap / 2);
  ...
  async function run(options = {}) {
    ...
    // Placement — deterministic packed-balloon, no physics.
    // 1. Bottom-up: compute subtreeRadius + ringRadius for every node.
    // 2. Top-down: place children around each parent at ringRadius,
    //    respecting half-plane constraint for non-root (outward arc).
    // 3. Single-pass cousin-overlap nudge for numerical drift.
    computeSubtreeRadius(_root);
    _root.x = 0;
    _root.y = 0;
    _root._direction = ROOT_DIRECTION_DEG;
    placeChildrenOf(_root, true);
    resolveCousinOverlaps();
  }

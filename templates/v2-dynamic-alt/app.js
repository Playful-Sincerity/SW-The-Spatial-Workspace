...
function createLayout({ baseSpacing = 180 } = {}) {
  ...
  async function run(options = {}) {
        ...
        for (let i = 0; i < kids.length; i++) {
          ...
          for (let j = i + 1; j < kids.length; j++) {
            ...
            if (PIN_HARD) {
              ...
              // velocity accumulation. applySubtreePosition respects fx/fy,
              // so pinned descendants stay put automatically — mixed subtrees
              // (e.g. carried parent with new children) are handled cleanly.
    ...
    // position. Only runs when pinStrength is below the hard-pin threshold
    // (hard pin uses fx/fy instead and skips this entirely).
...
function init() {
  ...
  // ─── TEMP TEST MARKER — dynamic-alt is currently a clean copy of v2 ───
  if (TREE && TREE.children) {
    TREE.children.push({
      name: "🧪 DYNAMIC-ALT TEST MARKER",
      path: "__test_marker_dynamic_alt__",
      type: "directory",
      children: [
        { name: "confirm-you-see-this.md", path: "__test_marker_dynamic_alt__/confirm-you-see-this.md", type: "file", content: "If you see this, you're viewing templates/v2-dynamic-alt (currently a clean copy of v2).", children: [] }
      ],
      meta: {}
    });
    console.log("[test-marker] dynamic-alt == v2 clean.");
  }
